...
proc propagate {cellpath otherInfo } {
    ...
    set log_type [get_property CONFIG.C_LOG_DATA_OFFLD $ip]
    ...
    set axi_clk_domain [get_property CONFIG.CLK_DOMAIN [get_bd_pins $ip/s_axi_aclk]]
     ...
     if { $en_log == 1 || $en_trace == 1} {
         ...
         if { ($log_type == "Memory map") } {
         set intf_in [get_bd_intf_pins ${ip}/S_AXI_OFFLD]
         set id_width [get_property CONFIG.ID_WIDTH $intf_in]
         set_property CONFIG.S_AXI_OFFLD_ID_WIDTH $id_width $ip
         }
        ...
        if { $log_type == "Stream"} {
           ...
           set axis_clk [get_property CONFIG.FREQ_HZ [get_bd_pins $ip/s_axi_offld_aclk]]
           set axis_clk_phase [get_property CONFIG.PHASE [get_bd_pins $ip/s_axi_offld_aclk]]
           set axis_clk_domain [get_property CONFIG.CLK_DOMAIN [get_bd_pins $ip/s_axi_offld_aclk]]
        }
...
proc post_propagate {cellpath otherInfo} {
    ...
    set log_type [get_property CONFIG.C_LOG_DATA_OFFLD $ip]
    set err_set 0
    if { ($en_log == 1 || $en_trace == 1) && $log_type == "Memory map" } {
       set_base_high $cellpath
       set id_width [get_property CONFIG.ID_WIDTH [get_bd_intf_pins $ip/S_AXI_OFFLD]]
       set_property  CONFIG.S_AXI_OFFLD_ID_WIDTH $id_width $ip
    }			 
    if { ($slot0_protocol == "AXI4LITE")} {
      set intf_in [get_bd_intf_pins ${ip}/SLOT_0_AXI4LITE]
    ...
    if { $err_set} {
       ...
       #########################################################
       One of the AXI4LITE slots have data width of more than 64
       #########################################################"
...
proc set_base_high {cellpath} {
  ...
  if { ($en_log == 1 || $en_trace == 1) && $log_type == "Memory map" } {
   # set ip [get_bd_cells $cellpath]
    ...
    Reg0 [list 0xA0000000 0xAFFFFFFF] \
    Reg [list 0xB0000000 0xBFFFFFFF] ]
 #puts "Setting the default values "
    set slave_bif [get_bd_intf_pins "$ip/S_AXI_OFFLD"]
