...
SC_MODULE(lsuctl) // LSU control
{
	...
	sc_stream_in <DMAS_T> s_dmas;
	sc_stream_out<DMAC_T> m_dmac;
...
#undef _trans
	...
	void start_of_simulation()
	{
		...
		sc_trace(tf, s_dmas, (std::string(name())+".s_dmas").c_str());
		sc_trace(tf, m_dmac, (std::string(name())+".m_dmac").c_str());
	}
#endif
...
SC_MODULE(mm2s)
{
	...
	sc_stream_in <DMAC_T> s_dmac;
	sc_stream_out<DMAS_T> m_dmas;

	axi_td_out m_dat;

	sc_stream<DMAC_T>  c_dmac;
	sc_stream<DMAS_T>  c_dmas;
	sfifo_cc<DMAC_T,2> u_dmac;
	sfifo_cc<DMAS_T,1> u_dmas;
	...
	sc_stream<TRANS_T>  c_transi;
	sc_stream<TRANS_T>  c_transo;
	sfifo_cc<TRANS_T,4> u_trans;
		...
		while (true) {
			...
			// cout << name() << "::ct_command ts: " << sc_time_stamp() << ", dmac: " << dmac << endl;
			{
				...
				while (size) {
					...
					tpay->get_extension<extension>()->qdepth = (c_transo.valid_r()) ? 1 : 0;
					...
					trans.tag  = dmac.tag;
					// cout << name() << "::ct_command ts: " << sc_time_stamp() << ", trans: " << trans << ", rdy: " << c_transi.ready_r() << endl;
					c_transi.write(trans);
	...
	{
		extern sc_trace_file *tf;
		sc_trace(tf, c_dmac, (std::string(name())+".c_dmac").c_str());
		sc_trace(tf, c_dmas, (std::string(name())+".c_dmas").c_str());
		sc_trace(tf, c_transi, (std::string(name())+".c_transi").c_str());
		sc_trace(tf, c_transo, (std::string(name())+".c_transo").c_str());
		sc_trace(tf, m_dat, (std::string(name())+".m_dat").c_str());
	}
#endif
	...
	SC_CTOR(mm2s) :
		c_dmac("c_dmac"),
		c_dmas("c_dmas"),
		u_dmac("u_dmac", DMAC_DEPTH),
		u_dmas("u_dmas", DMAS_DEPTH),
		peq("peq"),
		tpool(OUTSTANDING_READ+1),
		c_transi("c_transi"),
		c_transo("c_transo"),
		u_trans("u_trans", OUTSTANDING_READ)
	{
		u_dmac.clk(clk);
		u_dmac.reset(reset);
		u_dmac.s_port(s_dmac);
		u_dmac.m_port(c_dmac);

		u_dmas.clk(clk);
		u_dmas.reset(reset);
		u_dmas.s_port(c_dmas);
		u_dmas.m_port(m_dmas);

		u_trans.clk(clk);
		u_trans.reset(reset);
		u_trans.s_port(c_transi);
		u_trans.m_port(c_transo);
...
SC_MODULE(s2mm)
{
	...
	SC_CTOR(s2mm) :
		...
		c_transi("c_transi"),
		c_transo("c_transo"),
		u_trans("u_trans", OUTSTANDING_WRITE)
	{
		u_dmac.clk(clk);
		u_dmac.reset(reset);
		u_dmac.s_port(s_dmac);
...
SC_MODULE(lsu)
{
	...
	axi_tc c_ctlri, c_ctlro;
	axi_tc c_ctlwi, c_ctlwo;

	sc_stream<DMAC_T> c_dmacr;
	sc_stream<DMAC_T> c_dmacw;

	sc_stream<DMAS_T> c_dmasr;
	sc_stream<DMAS_T> c_dmasw;
