...
//	command.stat = 0;
	...
	/* send load command */
	paddr = ATRAN(src);
	command.cmd = LSU_ACMD(paddr,0,LSU_move); /* reqstat=0, command=move */
	...
	/* send store command */
	paddr = ATRAN(dst);
	command.cmd = LSU_ACMD(paddr,1,LSU_move); /* reqstat=1, command=move */
...
//	command.stat = 0;
	...
	/* send load command */
	paddr = ATRAN(src);
	command.cmd = LSU_ACMD(paddr,0,LSU_smove); /* reqstat=0, command=smove */
	command.tdest = gfwd_id+READ_CH;
	command.addr = paddr;
	command.inc = src_inc;
	stream_send(gport, &command, sizeof(command), F_BEGP|F_ENDP);
	...
	/* send store command */
	paddr = ATRAN(dst);
	command.cmd = LSU_ACMD(paddr,1,LSU_smove); /* reqstat=1, command=smove */
	command.tdest = gfwd_id+WRITE_CH;
	command.addr = paddr;
	command.inc = dst_inc;
	stream_send(gport, &command, sizeof(command), F_BEGP|F_ENDP);
