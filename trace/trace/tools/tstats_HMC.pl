...
while (<$fhi>) {
	...
	if ($type eq "W" || $type eq "R") {
		my $ram = (hex($address) < $opt_t) ? "SRAM" : "DRAM";
		if ($source == 1 && ($id == 2 || $id == 3)) {$source = 2;} # MCU
		my $tref = \%{$tab[$source]{$ram}{$type}};
		my $transport_ebit = ($source == 0) ? 10.3e-12 : 0.0; # Off-chip : On-chip
		my $access_ebit = ($ram eq "SRAM") ? 1.0e-12 : 19.4e-12; # SRAM : DRAM
		$tref->{ACCESS}++;
		# Narrow access model
...
if ($opt_r > 0) {
	...
	my $tot_bytes2  = 0;
	my $tot_energy2 = 0.0;
	print "Path,Access,Bytes(narrow),Joules(narrow),Bytes(HMC),Joules(HMC)", "\n";
	for my $source (0..2) {
		my $mast_access  = 0;
...
if ($opt_r > 1) {
	...
	for my $src (0..2) {
		print $master[$src], ",",
			$tab[$src]{"SRAM"}{"W"}{ACCESS} + $tab[$src]{"DRAM"}{"W"}{ACCESS}, ",",
			$tab[$src]{"SRAM"}{"R"}{ACCESS} + $tab[$src]{"DRAM"}{"R"}{ACCESS}, ",",
			$tab[$src]{"SRAM"}{"W"}{BYTES1} + $tab[$src]{"DRAM"}{"W"}{BYTES1}, ",",
			$tab[$src]{"SRAM"}{"R"}{BYTES1} + $tab[$src]{"DRAM"}{"R"}{BYTES1}, "\n";
	}
}
...
if ($opt_r > 2) {
	...
	print "LSU_SRAM_TranW/LSU_DRAM_TranR:",   ($tab[1]{"DRAM"}{"R"}{ACCESS} == 0) ? "NA" : $tab[1]{"SRAM"}{"W"}{ACCESS} / $tab[1]{"DRAM"}{"R"}{ACCESS}, "\n";
